...
const selectedDateTime = ref<Date>(props.defaultValue)

onMounted(() => {
  if (props.defaultValue) {
    selectedDateTime.value = props.defaultValue
    value.value = selectedDateTime.value
  } else {
    selectedDateTime.value = value.value as Date
  }
})
...
watch(selectedDateTime, (_) => {
  value.value = selectedDateTime.value
  if (selectedDateTime.value !== props.defaultValue) {
    emit('change', selectedDateTime.value)
  }
})
</script>
...
.container {
  ...
  .custom-datetime-picker:has(.el-input__wrapper.is-focus)
    + .title-container
    .title {
